    ...
    if (error) {
        return (
            <div className="min-h-screen pt-20 px-6 flex flex-col items-center justify-center text-center">
                ...
                <Button onClick={() => queryClient.invalidateQueries({ queryKey: ['doctorDashboard'] })}>Retry</Button>
            </div>
        );
    }
        ...
        <div className="min-h-screen pt-20 pb-12 px-6 bg-gradient-to-br from-[#F0F9FF] via-white to-[#F8FAFC]">
            <div className="max-w-5xl mx-auto">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                    <h1 className="text-3xl font-bold text-[#0F172A] mb-2">Clinic Availability</h1>
                    <p className="text-[#64748B]">Configure your weekly consultation windows</p>
                </motion.div>

                <div className="grid lg:grid-cols-4 gap-6">
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
                        <Card className="p-4 border border-gray-100 bg-white">
                            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4 px-2">Work Days</h3>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-3">
                        <Card className="p-8 border border-gray-100 bg-white shadow-sm">
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    <h2 className="text-2xl font-black text-[#0F172A]">{selectedDay}</h2>
                                    <p className="text-sm text-[#64748B] font-medium">
                                        {(availability[selectedDay] || []).length} Slots Active
                                    </p>
                                </div>
                                <div className="flex gap-2">
                                    <Button variant="ghost" size="sm" className="text-gray-400 hover:text-rose-500 font-bold" onClick={clearDay}>Clear</Button>
                                    <Button variant="ghost" size="sm" className="text-[#0EA5E9] font-bold" onClick={selectAll}>Select All</Button>
                                </div>
                            </div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-8">
                    <Card className="p-8 border border-gray-100 bg-white">
                        <h2 className="text-lg font-black text-[#0F172A] mb-6">Weekly Distribution</h2>
                            ...
                            {days.map((day) => {
                                ...
                                return (
                                    <div key={day} className="text-center group">
                                        <p className="text-[10px] font-black text-gray-400 mb-3 group-hover:text-[#0EA5E9] transition-colors">{day.slice(0, 3).toUpperCase()}</p>
