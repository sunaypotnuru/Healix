    ...
    if (error) {
        return (
            <div className="p-12 text-center bg-red-50 rounded-2xl border border-red-100">
                ...
                <h3 className="text-lg font-bold text-red-900">Failed to load doctors</h3>
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-[#0F172A] mb-1">Doctor Management</h1>
                    <p className="text-[#64748B]">Approve credentials and manage doctor accounts</p>
                </div>
            </div>
                                ...
                                <tr key={doctor.id} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="p-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-bold text-[#0F172A]">{doctor.name || 'Anonymous Doctor'}</p>
                                                <div className="flex items-center gap-3 text-xs text-gray-500 mt-0.5">
                                                    <span className="flex items-center gap-1"><Mail className="w-3 h-3" /> {doctor.email}</span>
                                                    <span className="flex items-center gap-1"><Phone className="w-3 h-3" /> {doctor.phone || 'No phone'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-4">
                                        <p className="font-medium text-[#0F172A] text-sm capitalize">{doctor.specialty || 'General'}</p>
                                        <p className="text-xs text-gray-500">{doctor.experience_years ? `${doctor.experience_years} years exp.` : 'Exp not specified'}</p>
                                    </td>
                                    <td className="p-4">
                                        {doctor.is_verified ? (
