    ...
    if (error) {
        return (
            <div className="min-h-screen pt-20 px-6 flex flex-col items-center justify-center text-center bg-gray-50">
                ...
                <h2 className="text-2xl font-bold text-[#0F172A] mb-2">Unable to load history</h2>
                <p className="text-[#64748B] max-w-md mb-6">{(error as Error).message}</p>
                <Button onClick={() => queryClient.invalidateQueries({ queryKey: ['patientHistory'] })} className="bg-[#0D9488] hover:bg-[#0F766E] text-white">Try Again</Button>
            </div>
        );
    }
        ...
        <div className="min-h-screen pt-20 pb-12 px-6 bg-gradient-to-br from-[#F0FDFA] via-white to-[#F8FAFC]">
            <div className="max-w-3xl mx-auto">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                    ...
                    <p className="text-[#64748B]">Your complete health timeline — scans, consultations, and prescriptions</p>
                </motion.div>
                    ...
                    <div className="space-y-6">
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <h3 className="font-semibold text-[#0F172A]">{record.type}</h3>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    {record.doctor && (
                                                        <span className="text-sm font-medium text-[#0F172A] hidden sm:block">{record.doctor}</span>
                                                    )}
                                            ...
                                            >
                                                {isScan ? (
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            <div>
                                                                <p className="text-xs text-[#64748B] uppercase font-bold">AI Confidence</p>
                                                                <p className="text-sm font-semibold text-[#0D9488]">{record.confidence}</p>
                                                            </div>
                                                            {record.hemoglobin_level && (
                                                                <div>
                                                                    <p className="text-xs text-[#64748B] uppercase font-bold">Estimated Hb</p>
                                                    ...
                                                    <div className="space-y-4">
                                                        <div>
                                                            <p className="text-xs font-bold text-[#64748B] uppercase mb-1">Consultation Summary</p>
                                                        ...
                                                        {record.prescription && (
                                                            <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                                                                <p className="text-xs font-bold text-[#64748B] uppercase mb-2 flex items-center gap-1">
                                                                    <FileText className="w-3 h-3" /> Digital Prescription
                                                                </p>
                                                                <p className="text-sm text-[#0F172A] whitespace-pre-line">{record.prescription}</p>
                                                            </div>
                                                        )}
                                                        <div className="flex items-center gap-2 text-sm text-[#64748B]">
                                                            <span className="font-semibold text-[#0F172A]">{record.specialty}</span>
                                                            <span>•</span>
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-6">
                                                    ...
                                                    <Button variant="outline" size="sm" className="h-9 px-4 text-[#22C55E] border-[#22C55E]/30 hover:bg-[#22C55E]/5 hover:border-[#22C55E]">
                                                        <Share2 className="w-3.5 h-3.5 mr-2" /> Share via WhatsApp
                                                    </Button>
                                                </div>
                                            </motion.div>
                                        )}
                        ...
                        {records.length === 0 && (
                            <Card className="p-12 text-center border-dashed border-2 bg-white/50">
                                ...
                                <h3 className="text-lg font-bold text-[#0F172A] mb-1">No history yet</h3>
                                <p className="text-[#64748B] mb-6">Start your first scan or book a consultation to see records here.</p>
                                <Button onClick={() => window.location.href = '/patient/scan'} className="bg-[#0D9488] text-white">Perform AI Scan</Button>
                            </Card>
                        )}
                    </div>
                </div>
